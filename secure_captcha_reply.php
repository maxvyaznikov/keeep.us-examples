...
<body class="captcha-example" onload="formatJson('server_answer')">
    ...
    curl_setopt($curl, CURLOPT_URL, "https://keeep.us/captcha/check/{$site_token}/?". strtotime("now"));
    ...
    curl_setopt($curl, CURLOPT_VERBOSE, 5);
    ...
    if ($err) {
        echo '<span class="failure">curl error occured: '. $err .'</span><br>';

        rewind($stderr);
        echo '<pre>'. stream_get_contents($stderr) .'</pre>';
        fclose($stderr);
    } else {
